...
export default function ImageCRUD() {
    ...
    try {
      ...
      if (file) {
        ...
        const response = await fetch(
          `https://api.cloudinary.com/v1_1/dzln4e8jb/image/upload`,
          {
            method: 'POST',
            body: formData
          }
      ...
      // FIX: Save price WITHOUT ₹ symbol
      // This prevents ₹₹ from appearing later.
      data.price = String(data.price || '').replace(/₹/g, '');

      // Debug log for serviceType
      ...
      // Explicit reset with serviceType
      reset({ serviceType: '' });
      setFile(null);
      fetchDesigns();
  ...
  const handleEdit = (design) => {
    ...
    // Remove all ₹ symbols while editing
    setValue(
      'price',
      design.price ? String(design.price).replace(/₹/g, '') : ''
    );

    setValue('tagsInput', design.tags ? design.tags.join(', ') : '');
    setValue('serviceType', design.serviceType || '');
    setFile(null);
  };
      ...
      <div className="bg-white p-6 rounded-lg shadow-md">
        ...
        >
          ...
          <input
            ...
            placeholder="Title (e.g., Festive Wristband)"
            className="p-2 border rounded-md"
          />
          ...
          <input
            {...register('tagsInput')}
            placeholder="Tags (comma-separated, e.g., Festival, Traditional)"
            className="p-2 border rounded-md md:col-span-2"
          />
